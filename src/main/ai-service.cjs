...
async function expandPrompt(visitorInput) {
  if (!process.env.GEMINI_API_KEY) {
    return {
      recipe: createFallbackRecipe(visitorInput),
...
Visitor input: "${visitorInput}"`;
  ...
  const result = await withTimeout(
    geminiGenerateContent(PROMPT_MODEL, [{ role: 'user', parts: [{ text: instruction }] }], {
...
async function generateImage(recipe) {
  ...
  if (imageProvider === 'openai' && process.env.OPENAI_API_KEY) {
    return generateOpenAiImage(recipe);
  }

  if (!process.env.GEMINI_API_KEY) {
    return {
      imageDataUrl: createSvgDataUrl(recipe),
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="1920" height="1080" viewBox="0 0 1920 1080">
    <defs>
      <radialGradient id="g" cx="50%" cy="42%" r="75%">
        ...
        <stop offset="42%" stop-color="${palette[1]}" stop-opacity="0.75"/>
    ...
    <g opacity="0.34" filter="url(#blur)">
      <circle cx="340" cy="240" r="180" fill="${palette[3]}"/>
      <circle cx="1510" cy="330" r="240" fill="${palette[2]}"/>
      <circle cx="980" cy="780" r="260" fill="${palette[1]}"/>
    </g>
    <g opacity="0.22" stroke="${palette[3]}" stroke-width="2" fill="none">
      ${Array.from({ length: 20 }, (_, i) => `<path d="M ${i * 105 - 80} 1040 C ${460 + i * 12} ${760 - i * 11}, ${920 - i * 9} ${460 + i * 8}, ${2020 - i * 82} 120"/>`).join('')}
    </g>
    <g opacity="0.28">
      ${Array.from({ length: 90 }, (_, i) => `<circle cx="${(i * 137) % 1920}" cy="${(i * 251) % 1080}" r="${2 + (i % 8)}" fill="${palette[(i % palette.length)]}"/>`).join('')}
    </g>
    <text x="80" y="930" fill="${palette[3]}" font-family="Arial, sans-serif" font-size="44" opacity="0.4">${title}</text>
    <text x="80" y="986" fill="${palette[3]}" font-family="Arial, sans-serif" font-size="22" opacity="0.26">${mood}</text>
  </svg>`;
