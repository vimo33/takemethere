  ...
  <main class="operator-shell">
    <section class="panel control-panel">
      ...
      <div class="button-grid">
        ...
        <button id="regenerate">Regenerate image</button>
        <button id="fallback">Skip to fallback</button>
        <button id="arrival">Trigger arrival</button>
      ...
      <div class="button-row">
        ...
        <button class="ghost" id="focus-output">Focus output</button>
        <button class="ghost" id="fullscreen-output">Output fullscreen</button>
        <button class="ghost" id="reload-output">Reload output</button>
      </div>

      <label>
        <h3>Output layout</h3>
        <select id="layout">
          <option value="three-wall">3 walls: left / front / right</option>
          <option value="ceiling">4 views: left / front / right / ceiling</option>
        </select>
      </label>
    </section>
    ...
    <section class="panel status-panel">
      ...
      <article class="recipe-card">
        ...
        <p id="transcript-value">No prompt yet.</p>
      </article>
      ...
      <article class="recipe-card">
        <h3>Lighting / audio cue</h3>
        <p id="cue-value">portal_breathing / idle_hum</p>
      </article>
    </section>
  </main>
`;
...
els.generate.addEventListener('click', () => {
  ...
  api.generateWorld(els.prompt.value);
});
...
els.blackout.addEventListener('click', () => {
  audio.mute();
  api.setState('BLACKOUT');
});
...
function renderSession(session) {
  ...
  if (['PORTAL_OPENING', 'ARRIVAL', 'WORLD_ACTIVE'].includes(state.key)) {
    audio.start(recipe.sound_style || 'dream');
  }
  if (['EXIT', 'RESET', 'BLACKOUT', 'IDLE'].includes(state.key)) {
    audio.mute();
  }
}
...
function startSpeechRecognition() {
  ...
  recognition.onerror = () => {
    els.errorSlot.innerHTML = '<div class="error">Transcription failed. Type the prompt manually.</div>';
  };

  recognition.onend = () => {
    if (els.prompt.value.trim()) api.generateWorld(els.prompt.value);
  };
